...
CREATE TABLE competitions (
    ...
    FOREIGN KEY (pool_id) REFERENCES pools(pool_id) ON DELETE RESTRICT
);
...
-- 7. COMPETITION_GROUPS
CREATE TABLE competition_groups (
    group_id INT AUTO_INCREMENT PRIMARY KEY,
    age_group_id INT NOT NULL,
    gender ENUM('M','F') NOT NULL,
    UNIQUE(age_group_id,gender),
    FOREIGN KEY (age_group_id) REFERENCES age_groups(age_group_id) ON DELETE RESTRICT
);
...
CREATE TABLE registrations (
    ...
    FOREIGN KEY (competition_id) REFERENCES competitions(competition_id) ON DELETE RESTRICT,
    FOREIGN KEY (style_id) REFERENCES styles(style_id) ON DELETE RESTRICT,
    FOREIGN KEY (distance_id) REFERENCES distances(distance_id) ON DELETE RESTRICT,
    FOREIGN KEY (group_id) REFERENCES competition_groups(group_id) ON DELETE RESTRICT
);
...
CREATE TABLE heats (
    ...
    UNIQUE(competition_id,group_id,round_id,heat_number)
);
...
CREATE TABLE results (
    ...
    FOREIGN KEY (heat_assignment_id) REFERENCES heat_assignments(heat_assignment_id) ON DELETE CASCADE
);

-- 15. PARTICIPANT_ADDRESSES
CREATE TABLE participant_addresses (
    participant_address_id INT AUTO_INCREMENT PRIMARY KEY,
    participant_id INT NOT NULL,
    address_id INT NOT NULL,
